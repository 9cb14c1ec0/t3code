...
> - **Optional `t3code/`-free branch names.** Settings → General has **Omit t3code/ from branch names**; when on, new worktree and pull-request checkout branches skip that prefix. Existing branches keep their names.
> - **Forgejo and Gitea as separate source control providers** — Forgejo/Codeberg via the `fj` CLI, Gitea via the `tea` CLI. Clone, detect remotes, and create or check out pull requests on each. The dedicated Pull requests inbox is still GitHub / GitLab / Bitbucket / Azure DevOps only.
