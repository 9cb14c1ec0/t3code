...
// Forgejo's pulls list cannot filter by head branch, so we over-fetch recently updated PRs
  ...
  return ForgejoApi.of({
    listPullRequests: (input) =>
      Effect.gen(function* () {
        ...
        // Fetch full pages (not the caller's `limit`): Forgejo can't filter by head branch,
